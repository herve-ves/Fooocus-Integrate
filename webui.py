    ...
    with gr.Row():
            ...
            with gr.Row(visible=False) as image_input_panel:
                with gr.Tabs():
                    with gr.TabItem(label='Inpaint or Outpaint (beta)') as inpaint_tab:
                        inpaint_input_image = grh.Image(label='Drag above image to here and draw mask', source='upload', type='numpy', tool='sketch', height=500, brush_color="#FFFFFF", elem_id='inpaint_canvas')
        ...
        with gr.Column(scale=1, visible=modules.path.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Setting'):
                ...
                image_number = gr.Slider(label='Image Number', minimum=1, maximum=8, step=1, value=modules.path.default_image_number)
